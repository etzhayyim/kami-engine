...
| **kami-live** | data | `scene::DanceScene::from_edn` — CLJ/EDN **VRM dance-scene** authoring (ADR-0043): the whole `LiveShow` is EDN — `:dance/{show,avatar,crowd,lighting,vj,setlist}` → deterministic show (beat grid + setlist + `DancePose` + crowd + lighting + VJ) + `AvatarBinding`. `render::show_to_render_ir_edn` projects each frame's snapshot → the existing EDN render-IR (`{:globals :instances}`, ADR-0041) so the dance draws on native (kami-webgpu-rs) + web (CLJS). `:dance/triggers` declare reactions (`director::Director`); `DanceScene::frame(dt) -> DanceFrame {render_ir, actions}` is the single per-frame host entry composing tick→resolve→render-IR. `lint::lint_scene` flags what the tolerant loader silently corrects (unknown enums / out-of-range / empty setlist / dangling trigger tags / beat-0 cues). `--bin kami-dance <scene.edn>` loads+lints+runs the show headless (no GPU; deterministic verify). `:dance/live2d` binds a **Live2D (2D Cubism) performer** (`live2d::Live2DBinding`, ADR-0045) driven by the *same* beat-synced pose → standard Cubism params (`DanceFrame.live2d`); the VRM render-IR emits the avatar as a skinned `:meshes`+`:lights`+`:materials` rig (ADR-0044). Re-uses kami-scene accessors; avatar mesh load stays on the VRM surface (`run_embed_vrm`, ADR-0031). Reference scene: `kami-clj-play3d/games/dance/`. **Full `:dance/*` + render-IR vocabulary reference: `kami-live/README.md`.** |
| **kami-vehicle-scene** | data | `kami-vehicle` made data-driven (ADR-0038): the 8-surface grip table + demo-circuit zone map (`data/ground.edn`) and the garage / powertrain / tire tuning (`data/garage.edn`) live as canonical EDN, loaded into the real engine structs via `from_edn` / `map_from_edn` / `build_from_edn` (mirrors `kami-live`). Hot physics stays native — only init/build-time CONFIG moves to EDN. `kami-vehicle`'s hardcoded `SurfaceKind` / `demo_circuit()` / `build()` stay as the `builtin()` fallback **and** the parity oracle: every shipped value is asserted `==` the Rust it mirrors (17 tests across surface/map/garage/vehicle parity, via `cargo test-native`). `kami-app-car-sim` (driver.etzhayyim.com) builds **both** its circuit and its car from this EDN at boot. Re-uses kami-scene accessors. See `kami-vehicle-scene/README.md`. |
...
| **kami-skeleton** | Skeletal animation (bone hierarchy, GPU skinning). Interp modes Linear/Step/CubicSpline, `evaluate_blend`/`evaluate_crossfade` (AnimationMixer-style), `solve_ik_ccd` (CCD IK), `AnimationClip::retarget` (by bone name) — ADR-0044 phase 4. EDN-authored clips (`clip_from_edn`) live in `kami-skeleton-scene` (the `-scene` data-tier; kami-skeleton stays kami-scene-free, ADR-0038). |
