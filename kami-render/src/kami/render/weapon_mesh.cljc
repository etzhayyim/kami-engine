...
(def contract :kotoba.render/weapon-mesh-v1)
...
(defn- component-specs [segments variant]
  (let [{:keys [stock-scale handguard-scale optic-height muzzle-scale]} variant]
    {:receiver {:shape :box :size [0.20 0.22 0.62] :center [0.0 0.0 0.20] :material-role :weapon}
     :barrel {:shape :cylinder :radius 0.045 :length 0.72 :center-z 0.86
              :segments segments :material-role :weapon}
     :stock {:shape :box :size [0.17 0.20 (* 0.46 stock-scale)]
             :center [0.0 0.0 -0.34] :material-role :weapon}
     :grip {:shape :box :size [0.11 0.32 0.16] :center [0.045 -0.23 -0.01]
            :material-role :accent}
     :magazine {:shape :box :size [0.13 0.34 0.19] :center [0.0 -0.25 0.28]
                :material-role :weapon}
     :optic {:shape :box :size [0.11 (* 0.13 optic-height) 0.24]
             :center [0.0 0.18 0.30] :material-role :emissive}
     :muzzle {:shape :cylinder :radius 0.066 :length (* 0.17 muzzle-scale) :center-z 1.30
              :segments segments :material-role :emissive}
     :handguard {:shape :box :size [(* 0.22 handguard-scale) 0.20 0.46]
                 :center [0.0 0.0 0.65] :material-role :accent}}))

(defn- component-mesh [{:keys [shape size center radius length center-z segments]}]
  (case shape
    :box (transform-box (mesh/cube) size center)
...
(defn- append-component [{:keys [positions normals uvs indices ranges] :as acc}
                         component-id spec]
  (let [generated (component-mesh spec)
...
(defn resolve-weapon
  ...
  (let [policy (get tier-policy tier)]
    ...
    (let [seed (equipment/stable-seed entity-id)
          ...
          specs (component-specs (:segments policy) variant)
          enabled (:enabled policy)
          selected (filter enabled component-order)
          assembled (reduce (fn [acc id] (append-component acc id (get specs id)))
