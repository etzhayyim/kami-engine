...
| CAE | 4/5 | Solver-neutral revision/provenance graph; 1D bar, 2D truss, 3D tetrahedral static FEM; thermal and modal; analytic/reaction/energy/patch/convergence tests; actual CalculiX 2.16 FRD cross-check with pinned input/result/image digests; injected `kotoba-lang/num` device-resident PCG with CPU parity and CUDA dispatch contract; native CUDA Runtime/cuBLAS/cuSPARSE/JNA bridge and NVIDIA workflow | Real NVIDIA runner evidence, multi-case independent-solver corpus, contact and mesh-control adapter, published qualification report |
