...
| CAE | 4/5 | Solver-neutral revision/provenance graph; 1D bar, 2D truss, 3D tetrahedral static FEM; thermal/modal and CalculiX cross-check; injected `kotoba-lang/num` device-resident PCG; native CUDA/cuBLAS/cuSPARSE/JNA/NVRTC path; `kotoba-lang/compiler` typed KIR emits sealed WGSL/CUDA/MSL artifacts; native Metal C ABI/JNA owns `MTLBuffer` lifecycle, runtime-compiles generated MSL, routes GEMV/GEMM through MPS, and passes the full 14-operation contract on Apple M1 Max | Real NVIDIA runner evidence, multi-case independent-solver corpus, contact/mesh-control adapter and published qualification report |
