...
| CAE | 4/5 | Solver-neutral revision/provenance graph; 1D bar, 2D truss, 3D tetrahedral static FEM; thermal/modal and CalculiX cross-check; injected `kotoba-lang/num` device-resident PCG; native CUDA/cuBLAS/cuSPARSE/JNA/NVRTC path; `kotoba-lang/compiler` typed KIR emits sealed WGSL/CUDA/MSL artifacts; native `MetalBackend` defines Metal/MPS ports and the generated MSL add/reduction hardware gate passes on Apple M1 Max while WGSL→Metal retains the full hardware contract | Real NVIDIA runner evidence, complete native-MPS hardware contract, multi-case independent-solver corpus, contact/mesh-control adapter and published qualification report |
