...
| CAE | 4/5 | Solver-neutral revision/provenance graph; 1D bar, 2D truss, 3D tetrahedral static FEM; thermal and modal; analytic/reaction/energy/patch/convergence tests; actual CalculiX 2.16 cross-check; injected `kotoba-lang/num` device-resident PCG with CPU/CUDA dispatch parity; native CUDA Runtime/cuBLAS/cuSPARSE/JNA bridge; `kotoba-lang/compiler` typed accelerator KIR deterministically generates sealed WGSL/CUDA elementwise/reduction artifacts and CUDA provenance binds their hashes | Real NVIDIA runner evidence, multi-case independent-solver corpus, contact and mesh-control adapter, published qualification report |
