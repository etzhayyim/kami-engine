...
| Assemblies | 4/5 | Deterministic mates and conflict/DOF diagnostics; cyclic point-mate projection with residual/conflict reporting; configurations; nested stable paths; revolute/prismatic joints, limits and gear/rack coupling; interference/mass properties; canonical stable-identity exchange; 1,000-occurrence data gate | General rotational nonlinear 6-DOF closed-loop mates and interactive public reference assembly |
| Manufacturing drawings | 4/5 | Associative parameter regeneration with stable unrelated annotations; section/detail/auxiliary/exploded views; classified hidden lines; semantic dimensions/GD&T/BOM/validated balloons; deterministic SVG, DXF and vector PDF; ISO A3/A4 and ASME ANSI-A/B templates | Curved-body HLR, golden external standards/vector corpus and full public regeneration E2E |
| CAE | 4/5 | Solver-neutral revision/provenance graph; 1D bar, 2D truss, 3D tetrahedral static FEM; thermal and modal; analytic/reaction/energy/patch/convergence tests; actual CalculiX 2.16 cross-check; injected `kotoba-lang/num` device-resident PCG with CPU/CUDA dispatch parity; native CUDA Runtime/cuBLAS/cuSPARSE/JNA bridge; `kotoba-lang/compiler` typed accelerator KIR emits sealed WGSL/CUDA artifacts whose hashes enter provenance; JNA compiles verified CUDA source with NVRTC, loads cached Driver API functions and contract-tests zero bootstrap kernel calls | Real NVIDIA runner evidence, multi-case independent-solver corpus, contact and mesh-control adapter, published qualification report |
